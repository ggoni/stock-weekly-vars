...
import pandas as pd
import numpy as np
from get_yahoo_loser_tickers import get_yahoo_finance_losers
...
def main():
    print("Stock weekly variation Z-Score Calculator for Yahoo Finance Losers")
    print("Fetching today's losing stocks...")

    # Get tickers from Yahoo Finance losers page
    tickers = get_yahoo_finance_losers()
    
    if not tickers:
        print("Error: Could not retrieve losing stocks. Please try again later.")
        return

    print(f"\nFound {len(tickers)} stocks to analyze")
